...
const ProductCard = ({ item, dgf, offerId }) => {
  ...
  discountPercentageCal = Math.round(
    (discountPrc * 100) / item.current_sale_price
  );
  ...
  const addtoWishlist = (productId) => {
    if (islogin) {
      ...
      axiosClient.post("user/wish/list/add", data).then(({ data }) => {
  ...
  return (
    <div className="product-card card position-relative my-3">
      {discountPercentageCal > 0 && (
        <span className="badge-custom">
          OFF<span className="box">{discountPercentageCal}%</span>{" "}
        </span>
      )}
      ...
      <div className="product-cart d-flex align-items-center justify-content-between p-2">
        <span className="product-price">
          {currency}
          {item.current_sale_price - discountPrc}
        </span>
        {discountPrc > 0 && (
          <del className="product-price-del">
            {currency}
            {Math.round(item.current_sale_price)}
          </del>
        )}
      ...
      <div className="action-bar position-absolute">
        <div className=" d-flex flex-column gap-15">
          ...
          <Link
            to=""
            className="iitemActioncard"
            onClick={() => showPrductDetails(item.id)}
          >
      ...
      >
        <Box sx={style}>{showid && <ProductDetailsCart id={showid} />}</Box>
      </Modal>
    </div>
  );
};
