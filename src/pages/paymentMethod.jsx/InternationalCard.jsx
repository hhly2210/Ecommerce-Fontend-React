...
const stripePromise = loadStripe(
  "pk_test_51MyBGiE9CkClsfMvxELwrlj251CRM87eB6Xsm59RIezr4rvJMIqW7XVY8dPNQU0Iy2BKrbBCZC7ArZjkWwGj872p00A2KxDThC"
);
...
function CheckoutForm(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (result.error) {
      ...
      axiosClient.post("stripe/payment", data).then(({ data }) => {
        if (data.status == 331) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      />
      <button type="submit" className="buy__btn w-100">
        Pay
      </button>
    </form>
  );
      ...
      <Container>
        ...
        {/* 
            <div>
                ...
                <div><button className="buy__btn w-100" onClick={cashonDelivery}><span> Pay </span></button></div>
            </div> */}
      </Container>
    </div>
  );
